...
class Zone_List_Table extends WP_List_Table {
    ...
    function column_default($item, $column_name){
        ...
        switch($column_name){
            case 'status':
                ...
                $return_image = '<p style="text-align:center"><a href="' . $url . '" title="'. __( 'Toggle featured', 'be_table_rate' ) . '">';
    ...
    function column_locations($item){
        ...
        $states = $woocommerce->countries->get_allowed_country_states();
        $return = '';
        switch ($item['zone_type']) {
            case 'everywhere':
                $return .= __('Everywhere','be_table_rate');

                $excluded = (isset($item['zone_except'])) ? sanitize_text_field($item['zone_except']) : '';
                if( isset( $excluded ) && $excluded != '' ) {
                    $i = 0;
                    $return .= '<br /><br/><i>'.__('Except for','be_table_rate').'...</i><br />';
            ...
            case 'countries':
                ...
                if( isset( $excluded ) && $excluded != '' ) {
                    $i = 0;
                    $cur_country = "";
                    $return_val .= '<br /><br/><i>'.__('Except for','be_table_rate').'...</i><br />';
                    ...
                    if(isset($item['zone_except']['postals']) && $item['zone_except']['postals'] != '') {
                        $return_val .= "Postal Codes: " . sanitize_text_field($item['zone_except']['postals']);
                    }
                }
                $return .= $return_val;
                break;
            case 'postal':
                ...
                if( isset( $excluded ) && $excluded != '' ) {
                    $i = 0;
                    $return .= '<br /><br/><i>'.__('Except for','be_table_rate').'...</i><br />'.sanitize_text_field($item['zone_except']);
                }
                break;
            default:
    ...
    function get_columns(){
        $columns = array(
            ...
            'title'     => __('Zone Title','be_table_rate'),
            'locations'    => __('Locations','be_table_rate'),
            'status'  => "<p style=\"text-align:center;margin:0;\">".__('Status','be_table_rate')."</p>"
            // 'order'  => __('Priority Order','be_table_rate')
        );
        return $columns;
    }
        ...
        if( 'delete'===$this->current_action() ) {
            if(isset($_GET['zone']) && is_numeric($_GET['zone'])) :
                if(!array_key_exists($_GET['zone'], $shipping_zones)) 
                    ...
                    update_option('be_jigoshop_shipping_zones', $shipping_zones);
    ...
    function tt_render_list_page(){
        ...
        <div class="wrap woocommerce">
            ...
            <div class="updated" style="font-weight:bold;">
                <p><?php _e('The selected zone(s) have been deleted.','be_table_rate'); ?></p>
            </div>
                <?php else: ?>
            <div class="error" style="font-weight:bold;">
                <p><?php _e('An error has occurred and the selected zone(s) were not deleted.','be_table_rate'); ?></p>
            </div>
                <?php endif; ?>
            <?php endif; ?>
    ...
    function tt_render_edit_page($zone_submit_id=''){
    ...
    <?php 
        ...
        if($method == 'edit' ) : 
            ...
            if(!array_key_exists($zoneID, $shipping_zones)) { echo "<p>Sorry, a zone with the given ID could not be found.</p>"; return; }
    ...
    ?>
            ...
            <form id="zone-editor" method="post" action="">
                ...
                <!-- Now we can render the completed list table -->
                <h3><?php echo __('Zone Details', 'be_table_rate'); ?></h3>
                <table class="form-table">
                <tr valign="top">
                    <th scope="row" class="titledesc"><label><?php echo __('Enabled', 'be_table_rate'); ?></label></th>
                ...
                <tr valign="top">
                    <th scope="row" class="titledesc"><label><?php echo __('Title', 'be_table_rate'); ?></label></th>
                ...
                <tr valign="top">
                    <th scope="row" class="titledesc"><label><?php echo __('Description', 'be_table_rate'); ?></label></th>
                ...
                <tr valign="top">
                    <th scope="row" class="titledesc"><label><?php echo __('Type', 'be_table_rate'); ?></label></th>
                    <td><select style="min-width:450px;" class="chosen_select" id="zone_type" name="zone_type">
                        <option value="everywhere"<?php if ($zone_fields['zone_type'] == 'everywhere') echo " selected=selected"; ?>><?php echo __('Everywhere', 'be_table_rate'); ?></option>
                        <option value="countries"<?php if ($zone_fields['zone_type'] == 'countries') echo " selected=selected"; ?>><?php echo __('Countries / States', 'be_table_rate'); ?></option>
                        <option value="postal"<?php if ($zone_fields['zone_type'] == 'postal') echo " selected=selected"; ?>><?php echo __('Postal Code', 'be_table_rate'); ?></option>
                    </select></td>
                    </tr>
                </table>
                <h3><?php echo __('Locations', 'be_table_rate'); ?></h3>
                <table id="location_everywhere" class="form-table" style="display:none">
                <tr valign="top">
                    <th scope="row" class="titledesc"><label><?php echo __('Countries', 'be_table_rate'); ?></label></th>
                    <td>All Allowed Countries as set on the 'General' tab </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row" class="titledesc"><label><?php echo __('Except', 'be_table_rate'); ?>...</label></th>
                        <td class="forminp">
                            <select multiple="multiple" name="location_everywhere_except[]" style="width:450px;" data-placeholder="<?php _e( 'Choose countries / states&hellip;', 'be_table_rate' ); ?>" title="Country" class="chosen_select">
                                <?php
                                    if ( $allowed_countries ) {
                                        $selections = ( isset( $zone_fields['zone_except'] ) ) ? explode(',', $zone_fields['zone_except']) : array();
                ...
                <table id="location_countries" class="form-table" style="display:none">
                    <tr valign="top">
                        <th scope="row" class="titledesc"><label><?php echo __('Countries', 'be_table_rate'); ?></label></th>
                        <td class="forminp">
                            <select multiple="multiple" name="location_countries[]" style="width:450px;" data-placeholder="<?php _e( 'Choose countries / states&hellip;', 'be_table_rate' ); ?>" title="Country" class="chosen_select">
                                <?php
                                    if ( $allowed_countries ) {
                    ...
                    <tr valign="top">
                        <th scope="row" class="titledesc"><label><?php echo __('Except the States/Provinces', 'be_table_rate'); ?>...</label></th>
                        <td class="forminp">
                            <select multiple="multiple" name="location_countries_exceptS[]" style="width:450px;" data-placeholder="<?php _e( 'Choose countries / states&hellip;', 'be_table_rate' ); ?>" title="Country" class="chosen_select">
                                <?php
                    ...
                    <tr valign="top">
                        <th scope="row" class="titledesc"><label><?php echo __('Except the Postal Codes', 'be_table_rate'); ?>...</label></th>
                        <td><textarea style="width:450px;height:75px" name="location_countries_except"><?php if(isset($zone_fields['zone_except']['postals'])) echo sanitize_text_field($zone_fields['zone_except']['postals']); ?></textarea><br ?>
                            <strong>,</strong> <small><?php _e('is used to separate all postal code entries','be_table_rate'); ?></small></br />
                            <strong>-</strong> <small><?php _e('is used to separate two postal codes in a range (numerical codes ONLY)','be_table_rate'); ?></small></br />
                            <strong>*</strong> <small><?php _e('is a wildcard used to represent multiple characters/numbers','be_table_rate'); ?></small></br />
                            <strong>^</strong> <small><?php _e('is used to denote postal codes or ranges to be excluded','be_table_rate'); ?></small></br /></td>
                    </tr>
                </table>
                <table id="location_postal_code" class="form-table" style="display:none">
                <tr valign="top">
                    <th scope="row" class="titledesc"><label><?php echo __('Country', 'be_table_rate'); ?></label></th>
                    <td class="forminp"><select name="location_country" style="width:450px;" data-placeholder="<?php _e( 'Choose a country&hellip;', 'be_table_rate' ); ?>" title="Country" class="chosen_select">
<?php
...
?>
                    ...
                    <tr valign="top">
                        <th scope="row" class="titledesc"><label><?php echo __('Postal Codes', 'be_table_rate'); ?></label></th>
                        <td><textarea style="width:450px;height:75px" name="location_codes"><?php if(isset($zone_fields['zone_postal'])) echo sanitize_text_field($zone_fields['zone_postal']); ?></textarea><br ?>
                            <strong>-</strong> <small><?php _e('is used to separate two postal codes in a range (numerical codes ONLY)','be_table_rate'); ?></small></br />
                            <strong>*</strong> <small><?php _e('is a wildcard used to representent multiple characters/numbers','be_table_rate'); ?></small></br />
                            <strong>^</strong> <small><?php _e('is used to denote postal codes or ranges to be excluded','be_table_rate'); ?></small></br /></td>
                    </tr>
                    <tr valign="top">
                        <th scope="row" class="titledesc"><label><?php echo __('Except', 'be_table_rate'); ?>...</label></th>
                        <td><textarea style="width:450px;height:75px" name="location_postal_except"><?php if(isset($zone_fields['zone_except'])) echo sanitize_text_field($zone_fields['zone_except']); ?></textarea><br ?>
                            <strong>-</strong> <small><?php _e('is used to separate two postal codes in a range (numerical codes ONLY)','be_table_rate'); ?></small></br />
                            <strong>*</strong> <small><?php _e('is a wildcard used to representent multiple characters/numbers','be_table_rate'); ?></small></br /></td>
                    </tr>
                </table>
